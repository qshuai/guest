...
pagingParam(10,'message');
$result = mysqli_query($con,"SELECT id,user_from,content,date,status FROM message WHERE user_to='{$_COOKIE['username']}' LIMIT $cite,$pagesize") or die(mysqli_error($con));
                  ...
                  <table>
                        <form action="?action=delete" method="post">
                              ...
                              <?php while($query = mysqli_fetch_array($result,MYSQLI_ASSOC)){?>
                                    ...
                                    <tr><td><?php echo $query['user_from'];?></td><td><a href="message_detail.php?action=<?php echo $query['id'];?>"><?php echo $read;?></a></td><td><?php echo $query['date'];?></td><td><?php echo $img;?></td><td><input type="checkbox" name="ids[]" value="<?php echo $query['id']?>"></td></tr>
                              <?php }?>
